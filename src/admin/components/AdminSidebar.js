...
// All nav items with their permission key
const ALL_ITEMS = [
...
export default function AdminSidebar() {
  ...
  useEffect(() => {
    // Read from sessionStorage (set by AdminRoute)
  ...
  }, [loc.pathname]); // re-read on route change
  ...
  return (
    <aside style={{
      ...
      >
        ...
        <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.58rem', color: '#8892b0', marginTop: 1, letterSpacing: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            ...
            {isAdmin
              ...
              : <UserCog size={13} style={{ color: '#7c3aed' }} />
            }
          </div>
          <div style={{ overflow: 'hidden' }}>
            <p style={{ color: '#e8eaf6', fontSize: '0.82rem', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {name}
            </p>
            <p style={{ color: isAdmin ? '#00d4ff' : '#7c3aed', fontSize: '0.62rem', fontFamily: 'JetBrains Mono', letterSpacing: 1 }}>
        ...
        {visibleItems.map(({ to, icon: Icon, label, badge }) => {
          ...
          return (
            ...
            >
              ...
              {label}
              {badge && (
                <span style={{ marginLeft: 'auto', width: 7, height: 7, borderRadius: '50%', background: '#ff6b00', flexShrink: 0 }} />
              )}
      ...
      <div style={{ padding: '0.75rem', borderTop: '1px solid rgba(0,212,255,0.1)' }}>
        <button onClick={logout} style={{
          ...
          display: 'flex', alignItems: 'center', gap: 10,
          fontFamily: 'Rajdhani,sans-serif', fontWeight: 600, fontSize: '0.87rem',
          transition: 'all 0.2s',
        }}
